...
// a class that holds a single hypothesis
  ...
  {
    os << hyp.getState() << " -- "
       << " ilabel=" << hyp.getArc().ilabel << " /"
       << " olabel=" << hyp.getArc().olabel << " :"
       << " weight=" << hyp.getArc().weight << " -->"
       << " next_state=" << hyp.getNextState() << " =="
       << " hyp_weight=" << hyp.getWeight() << " ["
       << " hyp_nr=" << hyp.getNumber() << "]";
    return os;
  }
...
// do a beam-search type trim, aligning the number of non-epsilon input symbols
  ...
  void BeamTrim(const Fst<Arc> &ifst, MutableFst<Arc> *ofst, unsigned beamWidth)
  {
    ...
    // get the current set
    HypothesisSet *GoodSet = new HypothesisSet;
    ...
    unsigned step = 1;
    while (GoodSet->size() > 0 && weightLess(GoodSet->begin()->getWeight(), finalSet->rbegin()->getWeight())) {

#ifdef BEAM_DEBUG
      cerr << "Starting iteration " << step << ", GoodSet->size() == " << GoodSet->size() << endl;
      for (typename HypothesisSet::iterator hypIt = GoodSet->begin(); hypIt != GoodSet->end(); hypIt++) {
      ...
      HypothesisSet *SymTransSet = new HypothesisSet;
      SymTransSet->insert(badHyp);

      // loop through all hypotheses in the current set
      while (GoodSet->size() > 0) {

        // pop the current hypothesis and make sure it is fit to be examined
        Hypothesis<Arc> currHyp = * GoodSet->begin();
#ifdef BEAM_DEBUG
        cerr << " examining " << currHyp << endl;
#endif
        GoodSet->erase(currHyp);
#ifdef BEAM_DEBUG
        cerr << " erased " << currHyp << endl;
#endif


        const Weight &worstWeight = SymTransSet->rbegin()->getWeight();
...
#endif
            (arc.ilabel ? SymTransSet : GoodSet)->insert(nextHyp);

          }
        }

        // trim the next set
        while (SymTransSet->size() > beamWidth) {
#ifdef BEAM_DEBUG
          cerr << "  Erasing " << *SymTransSet->rbegin() << endl;
#endif
          SymTransSet->erase(*SymTransSet->rbegin());
        }
      ...
      for (index = SymTransSet->begin(); index != SymTransSet->end() && GoodSet->size() < beamWidth; index++) {
        // pair<StateId,StateId> myPair(index->getState(), index->getNextState());
        // if(!hasStates.count(myPair)) {
        GoodSet->insert(*index);
        //    hasStates.insert(myPair);
        // }
      }

#ifdef BEAM_DEBUG
      cerr << " Hypothesis Count: before=" << SymTransSet->size() << ", after=" << GoodSet->size() << endl;
